...
async function syncToDevTo(items) {
  ...
  for (const item of items) {
    try {
      ...
      )} by [${author.name}](${author.link}) @ [ Blog](https://jiujuaner.com/)\n\n${
        item.markdown
      } `;
      const image = item.meta.image
        ? item.meta.image.startsWith('/')
          ? `https://jiujuaner.com${item.meta.image}`
          : item.meta.image
        : undefined;
  ...
  for (const blogFile of files) {
    try {
      ...
      if (data.meta.skipSync !== true) {
        items.push({
          slug: cleanName,
          canonical: `https://jiujuaner.com/blog/${cleanName}`,
    ...
    visit(tree, (node, index, parent) => {
      ...
      if (node.type === 'image') {
        if (node.url.startsWith('/')) {
          node.url = `https://jiujuaner.com${node.url}`;
        }
